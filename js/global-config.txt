    ...
    loadFromSessionStorage() {
        ...
        if (saved) {
            try {
                ...
                // Рассылаем ТОЛЬКО событие 'SessionRestore'. Модули сами достают
                // fullConfig.SettingsOrder и вызывают updateScenario(..., isRestore=true).
                // Ранее здесь отдельно рассылалось событие 'SettingsOrder', но модули
                // обрабатывают его с isRestore=false, что приводило к вызову
                // clearModuleData() и стиранию только что восстановленных данных
                // ДО того, как приходило само событие 'SessionRestore'.
                setTimeout(() => {
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (event) => {
                try {
                    ...
                    // Рассылаем ТОЛЬКО событие 'FileImport' (см. пояснение в loadFromSessionStorage
                    // про то, почему отдельное предварительное 'SettingsOrder' здесь вредно).
                    setTimeout(() => {
