export default class DocxGenerator {
    ...
    prepareAttributes(settingsOrder, questionnaire, teamAudit, processesModule, automatedSystems) {
        ...
        // Изменения команды (сценарий "Изменение")
        // ВАЖНО: Модуль 3 (TeamAudit) в saveChangeConfig() сохраняет include/exclude/signer/control
        // ПЛОСКО, прямо в корне конфигурации модуля (через updateModuleConfig), а НЕ во
        // вложенном объекте teamAudit.changes. Поэтому читаем их напрямую из teamAudit.
        if (teamAudit.include && teamAudit.include.length > 0) {
            attrs.addEmployees = teamAudit.include.map(emp => 
                `${emp.fullName}(${emp.tabNumber}),${emp.department},${emp.position},${emp.bank}`
            ).join(';');
        }
        if (teamAudit.exclude && teamAudit.exclude.length > 0) {
            attrs.delEmployees = teamAudit.exclude.map(emp => 
                `${emp.fullName}(${emp.tabNumber}),${emp.department},${emp.position},${emp.bank}`
            ).join(';');
        }
    ...
    /**
     * Строит ячейку заголовка таблицы, к тексту которой в конце добавлена
     * метка сноски Word (<w:footnoteReference>), например для слова "ресурса".
     * Сам текст сноски должен быть заранее собран в this.pendingFootnotes
     * и встроен в документ через injectFootnotes.
     */
    buildHeaderCellWithFootnote(text, footnoteId, { width, fill = null } = {}) {
        const rPr = `<w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:cs="Times New Roman"/><w:sz w:val="24"/><w:szCs w:val="24"/><w:b/></w:rPr>`;
        const footnoteRPr = `<w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:cs="Times New Roman"/><w:sz w:val="24"/><w:szCs w:val="24"/><w:b/><w:vertAlign w:val="superscript"/></w:rPr>`;

        const content = `<w:p><w:pPr><w:jc w:val="center"/>${rPr}</w:pPr>` +
            `<w:r>${rPr}<w:t xml:space="preserve">${this.escapeXml(text)}</w:t></w:r>` +
            `<w:r>${footnoteRPr}<w:footnoteReference w:id="${footnoteId}"/></w:r>` +
            `</w:p>`;
    ...
    buildAppendix3TableXml(items) {
        ...
        const COLW = {
            ...
            role: 950,  // Необходимая роль/группа доступа
    ...
    async generateDocument(dictAttributesJson, pathTaskFolder) {
        ...
        if (!this.flagIsChange) {
            ...
            // Дата направления акта (актуально для "проведение -> изменение");
            // раньше не парсилась, из-за чего пункт 2 распоряжения никогда не показывался
            if (dictAttributesJson['act_date'] && dictAttributesJson['act_date'].length > 1) {
                this.dateAct = dictAttributesJson['act_date'];
            } else {
                this.dateAct = null;
    ...
    async generatePrepOrExecuteAudit() {
        ...
        // Зам руководитель отображается только если он был выбран
        if (this.dataDeputyHeadAudit) {
            dictOrder.subsuper = `${deputyHeadFIO} – ${deputyHeadPost}`;
            dictOrder.showSubsuper = true;
        } else {
            dictOrder.subsuper = '';
        ...
        if (this.flagIsTB) {
            ...
            dictOrder.podrazdelenie = 'территориального банка';
        } else {
    ...
    async generateChangeAudit() {
        ...
        // Сквозная нумерация приложений: номер присваивается только тем приложениям,
        // которые реально показываются в документе (в порядке emp -> add -> rem -> aski).
        // Раньше номера при пропуске одного из приложений считались независимо друг от друга
        // и "разъезжались" (например, pril_aski не учитывал отсутствие Приложения emp/add/rem).
        const hasApp1 = !!(this.listDataEmployeesAdd || this.listDataEmployeesDel);
        const hasApp2 = !!(this.newListProcessesAudit && listDictProcessesAdd && listDictProcessesAdd.length > 0);
    ...
    /**
     * Добавляет реальные сноски Word в документ, напрямую дописывая нужные
     * части docx-архива (без платных модулей docxtemplater):
     * - word/footnotes.xml - текст самих сносок
     * - word/_rels/document.xml.rels - связь document.xml -> footnotes.xml
     * - [Content_Types].xml - декларация типа контента для footnotes.xml
     *
     * footnotes: [{ id: number, text: string }] - ссылки <w:footnoteReference w:id="..."/>
     * на эти id должны быть заранее вставлены в raw-XML (см. buildAppendix3TableXml)
     */
    injectFootnotes(zip, footnotes) {
        ...
        // 1. word/footnotes.xml
        const existingFootnotesFile = zip.file(FOOTNOTES_PATH);
        let footnotesXml = existingFootnotesFile ? existingFootnotesFile.asText() : null;
        if (!footnotesXml) {
            footnotesXml =
        ...
        const newEntries = footnotes.map(fn =>
            `<w:footnote w:id="${fn.id}"><w:p><w:pPr><w:jc w:val="both"/></w:pPr>` +
            `<w:r><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:cs="Times New Roman"/><w:sz w:val="20"/><w:vertAlign w:val="superscript"/></w:rPr><w:footnoteRef/></w:r>` +
            `<w:r><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:cs="Times New Roman"/><w:sz w:val="20"/></w:rPr>` +
            `<w:t xml:space="preserve"> ${this.escapeXml(fn.text)}</w:t></w:r></w:p></w:footnote>`
        ).join('');

        footnotesXml = footnotesXml.replace('</w:footnotes>', `${newEntries}</w:footnotes>`);
        zip.file(FOOTNOTES_PATH, footnotesXml);

        // 2. word/_rels/document.xml.rels - связь на footnotes.xml (если её ещё нет в шаблоне)
        const relsFile = zip.file(RELS_PATH);
        if (relsFile) {
            let relsXml = relsFile.asText();
            if (!relsXml.includes('footnotes.xml')) {
                const usedIds = [...relsXml.matchAll(/Id="rId(\d+)"/g)].map(m => parseInt(m[1], 10));
                const nextId = usedIds.length ? Math.max(...usedIds) + 1 : 1;
                const rel = `<Relationship Id="rId${nextId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/footnotes" Target="footnotes.xml"/>`;
                relsXml = relsXml.replace('</Relationships>', `${rel}</Relationships>`);
                zip.file(RELS_PATH, relsXml);
            }
        }

        // 3. [Content_Types].xml - декларация типа контента для footnotes.xml
        const ctFile = zip.file(CT_PATH);
        if (ctFile) {
            let ctXml = ctFile.asText();
            if (!ctXml.includes('/word/footnotes.xml')) {
                const override = '<Override PartName="/word/footnotes.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.footnotes+xml"/>';
                ctXml = ctXml.replace('</Types>', `${override}</Types>`);
                zip.file(CT_PATH, ctXml);
            }
        }
    }
    ...
    async renderTemplate(dictOrder, templatePath, taskName) {
        try {
            ...
            // Добавляем сноски (например, к таблице Приложения 3), если они были собраны
            if (this.pendingFootnotes && this.pendingFootnotes.length > 0) {
                this.injectFootnotes(zip, this.pendingFootnotes);
            }
